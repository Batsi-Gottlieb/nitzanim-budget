...
function ExpenseRow({ item, facilityId }: { item: ExpenseLineItem; facilityId: string }) {
  ...
  return (
    <form action={handleSave} className="flex flex-wrap items-end gap-2 rounded-xl border border-slate-200 bg-slate-50/60 p-3">
      <div>
        <label className="mb-1 block text-[11px] text-slate-500">קטגוריה</label>
        <input name="category" defaultValue={item.category} required className="w-44 rounded-lg border border-slate-200 px-2 py-1 text-sm" />
      </div>
      <div>
        <label className="mb-1 block text-[11px] text-slate-500">סכום חודשי</label>
        <input
          name="monthly_amount"
          type="number"
      ...
      <div className="min-w-0 flex-1">
        ...
        <input name="notes" defaultValue={item.notes ?? ""} className="w-full rounded-lg border border-slate-200 px-2 py-1 text-sm" />
      </div>
      <button
        disabled={isPending}
...
export function ExpensesSection({ facilityId, items }: { facilityId: string; items: ExpenseLineItem[] }) {
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-2xs">
      <h2 className="mb-1 text-sm font-bold text-slate-900">הוצאות שאינן שכר</h2>
      <p className="mb-3 text-xs text-slate-500">הוצאות חודשיות שוטפות של הפנימייה (מטבח, חשמל, אחזקה, שכ״ד וכו׳).</p>
      <div className="space-y-2">
        {items.map((item) => (
          <ExpenseRow key={item.id} item={item} facilityId={facilityId} />
        ))}
        {items.length === 0 && <p className="text-sm text-slate-500">אין עדיין הוצאות</p>}
      </div>

      <form action={handleAdd} className="mt-4 flex flex-wrap items-end gap-2 border-t border-slate-200 pt-4">
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">קטגוריה</label>
          <input name="category" required placeholder="לדוגמה: חשמל ומים" className="w-44 rounded-lg border border-slate-200 px-2 py-1.5 text-sm" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">סכום חודשי</label>
          <input name="monthly_amount" type="number" className="w-32 rounded-lg border border-slate-200 px-2 py-1.5 text-sm" />
        </div>
        <button
          disabled={isPending}
