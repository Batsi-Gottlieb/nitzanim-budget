...
import { createResellerCompanyWithAdmin } from "./actions";

type State = { error: string | null; email?: string; password?: string };

export function CreateResellerCompanyForm() {
  const [state, formAction, isPending] = useActionState<State, FormData>(
    async (_prev, formData) => createResellerCompanyWithAdmin(formData),
    { error: null }
  );
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-2xs">
      <form action={formAction} className="flex flex-wrap items-end gap-3">
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">שם החברה (רו״ח)</label>
          <input name="name" required className="w-56 rounded-lg border border-slate-200 px-2.5 py-1.5 text-sm" />
        </div>
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-500">אימייל מנהל החברה</label>
        ...
        >
          ...
          {isPending ? "יוצר..." : "פתיחת חברה חדשה"}
        </button>
      </form>
      {state.error && <p className="mt-3 text-sm text-red-600">{state.error}</p>}
      {state.password && (
        <div className="mt-3 rounded-lg border border-indigo-100 bg-indigo-50 p-3 text-sm">
          <p className="font-semibold text-slate-800">פרטי התחברות למנהל החברה (מסרו באופן מאובטח, אינם יוצגו שוב):</p>
